...
function Taches({ callClaude }) {
  ...
  const fetchTaches = async (key) => {
    ...
    const token = window.__CLICKUP_TOKEN__ || "";
    try {
      ...
      if (data.tasks) {
        ...
        const tasks = data.tasks.map(t => ({
          id: t.id,
          nom: t.name,
          statut: t.status?.status || "open",
          priorite: priorityMap[t.priority?.id] || "normal",
          date: t.due_date ? new Date(parseInt(t.due_date)).toLocaleDateString("fr-FR") : "—",
          description: t.description?.slice(0,120) || "",
        }));
        setTaches(t => ({ ...t, [key]:tasks }));
      } else {
        setError("Vérifiez votre clé ClickUp dans les variables Vercel.");
      }
    } catch(e) { setError("Erreur connexion ClickUp : " + e.message); }
    setLoading(false);
  };
